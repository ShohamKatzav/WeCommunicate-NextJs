...
export default class ConversationRepository {
    ...
    static async GetConversationById(conversationId: string) {
        try {
            const conversation = await Conversation.findOne({ _id: conversationId }).populate('members', 'email nickname avatarUrl accentColor');
            return conversation;
        } catch (error) {
    ...
    static async GetRecentConversations(user: Types.ObjectId, perDocumentLimit: number = 1) {
        try {
            ...
            const conversations = await Conversation.find({
                ...
                .populate('members', 'email nickname avatarUrl accentColor')
                .populate({
                    path: 'messages',
                    model: Message,
