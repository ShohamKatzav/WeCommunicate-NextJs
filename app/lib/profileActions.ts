...
import { getT, setLocaleCookie } from "@/app/i18n/server";
...
// PhoneNumberEditor.tsx and errors.noEmailForPhoneChange for why (phone-only
...
export const getProfile = async (identifier: string) => {
    const t = await getT();
    if (!identifier) return { success: false, error: t('errors.invalidUser') };
    try {
        await connectDB();
        const profile = await AccountRepository.getProfileByIdentifier(identifier) as { _id: { toString(): string }; lastSeen?: Date } | null;
        if (!profile) return { success: false, error: t('errors.userNotFound') };
        ...
        return { success: false, error: t('errors.loadProfileFailed') };
    }
}
...
export const updateMyProfile = async ({ nickname, about, accentColor, locale }: { nickname?: string; about?: string; accentColor?: string; locale?: string }) => {
    const t = await getT();
    try {
        await connectDB();
        const userID = await extractUserIDFromCoockie();
        ...
        const updates: { nickname?: string; about?: string; accentColor?: string; locale?: string } = {};

        if (nickname !== undefined) {
            const trimmed = nickname.trim();
            if (!trimmed) return { success: false, error: t('errors.nicknameRequired') };
            if (trimmed.length > 40) return { success: false, error: t('errors.nicknameTooLong') };
        ...
        if (accentColor !== undefined) {
            if (!ACCENT_COLORS.includes(accentColor as typeof ACCENT_COLORS[number])) {
                return { success: false, error: t('errors.invalidAccentColor') };
            }
            updates.accentColor = accentColor;
        }

        if (locale !== undefined) {
            if (!isLocale(locale)) return { success: false, error: t('errors.invalidLocale') };
        ...
        // The account and this device together, same as login does. Setting
        // the cookie re-renders the page, so the whole UI switches language
        // as this save returns.
        if (updates.locale && isLocale(updates.locale)) await setLocaleCookie(updates.locale);

        revalidatePath('/profile');
        revalidatePath('/chat');
        // Worded in the language just saved, not the one the request came in.
        const message = (updates.locale ? translatorFor(updates.locale) : t)('profile.edit.updated');
        return { success: true, profile: JSON.parse(JSON.stringify(profile)), message };
    } catch (err) {
        console.error('Failed to update profile:', err);
        return { success: false, error: t('errors.updateProfileFailed') };
    }
}
...
export const requestPhoneChangeOTP = async (newPhone: string) => {
    const t = await getT();
    if (!isPhone(newPhone)) return { success: false, error: t('errors.invalidPhone') };
    try {
        await connectDB();
        const userID = await extractUserIDFromCoockie();
        if (typeof userID !== 'string') return { success: false, error: t('errors.unauthorized') };
        ...
        if (!ownEmail) return { success: false, error: t('errors.noEmailForPhoneChange') };
    ...
    try {
        ...
        try {
            ...
            if ((err as { code?: number })?.code === 11000) {
                return { success: false, error: t('errors.phoneTaken') };
            }
            throw err;
        }
    } catch (err) {
        console.error('Failed to confirm phone change:', err);
        return { success: false, error: t('errors.updatePhoneFailed') };
    }
}
...
function resolveEmailChangeChannel(t: TFunction, requested: 'email' | 'sms' | undefined, hasRealEmail: boolean, hasPhone: boolean): { channel: 'email' | 'sms' } | { error: string } {
    const useSms = requested === 'sms';
    if (useSms) return hasPhone ? { channel: 'sms' } : { error: t('errors.noPhoneOnFile') };
    return hasRealEmail ? { channel: 'email' } : { error: t('errors.noEmailOnFile') };
}
...
export const requestEmailChangeOTP = async (newEmail: string, channel?: 'email' | 'sms') => {
    ...
    if (!isEmail(normalized)) return { success: false, error: t('errors.invalidEmail') };
    try {
        await connectDB();
        const userID = await extractUserIDFromCoockie();
        if (typeof userID !== 'string') return { success: false, error: t('errors.unauthorized') };
        ...
        if (!own) return { success: false, error: t('errors.accountNotFound') };
        const { account, hasRealEmail, hasPhone } = own;

        if (hasRealEmail && account.email!.trim().toLowerCase() === normalized) {
            return { success: false, error: t('errors.sameEmail') };
        }

        const resolved = resolveEmailChangeChannel(t, channel, hasRealEmail, hasPhone);
        ...
        if (existing.accountExists) return { success: false, error: t('errors.emailTaken') };
...
export const confirmEmailChangeStep1 = async (newEmail: string, otp: string, channel?: 'email' | 'sms') => {
    ...
    try {
        ...
        return { success: false, error: t('errors.verifyCodeFailed') };
    }
}
...
export const confirmEmailChange = async (newEmail: string, otp: string) => {
    ...
    try {
        ...
        if (!account) return { success: false, error: t('errors.accountNotFound') };
        ...
        try {
            ...
            if ((err as { code?: number })?.code === 11000) return { success: false, error: t('errors.emailTaken') };
            throw err;
        }
        if (!updated) return { success: false, error: t('errors.updateEmailFailed') };
        ...
        return { success: false, error: t('errors.updateEmailAddressFailed') };
    }
}
