...
const ConversationsBar =
    ({ isMobileChatsSidebarOpen,
        ...
        return (
            <div className={`
        ${isMobileChatsSidebarOpen ? 'translate-x-0' : 'max-xl:-translate-x-full max-xl:rtl:translate-x-full'}
      ...
      `}>
                <aside className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-transform duration-300`}>
                    <div className="p-4 flex items-center justify-between">
                        <h3 className="text-lg font-semibold">{t("chat.sidebar.chats")}</h3>
                        <button onClick={() => handleOpenModal('single')} aria-label={t("chat.sidebar.newConversation")} className="p-1 rounded-md bg-green-700 text-white">
                            <Plus size={25} />
                        </button>
                    </div>
                    ...
                    <div className="px-4 pb-3">
                        <div className="relative">
                            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("chat.sidebar.search")}
                                className="w-full ps-10 pe-3 py-2  bg-gray-100 dark:bg-gray-700
                                flex-1 p-3 rounded-lg focus:outline-none border"
                                aria-label={t("chat.sidebar.searchLabel")} />
                            <div className="absolute start-3 top-2.5 text-muted-foreground"><Search size={16} /></div>
                        </div>
                    </div>
              ...
              transition-all duration-200 font-semibold"
                        >
                            ...
                            <span>{t("chat.sidebar.createGroup")}</span>
                        </button>
                    </div>
